...
## Why
The project is moving from prototype toward a first release candidate. The current base supports Flask UI, upload, scheduler, FFmpeg conversion, MPV playback, and composite output, but V1 requires real GPIO/MOSFET safety, first-boot reliability, release validation documents, and production-grade error handling.

## What Changes
- Implement real GPIO17 MOSFET control while keeping all GPIO access inside `gpio.py`.
- Ensure MOSFET power is disabled on playback stop, playback completion, application exit, service stop, reboot, SIGTERM, Ctrl+C, exceptions, and unexpected failures.
...
- Hardware affected: GPIO17, MOSFET-controlled VTX/fan branch
...
- Multiple playlists
- New hardware beyond GPIO17 MOSFET VTX/fan switching
